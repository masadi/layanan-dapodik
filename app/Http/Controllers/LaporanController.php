    ...
    public function update_jam(){
        ...
        $jam_ptk = Jam::with(['ptk' => function($query) use ($absen){
            $query->whereHas('hari', function($query) use ($absen){
                $query->where('jam_hari.nama', Carbon::create($absen->created_at->format('Y-m-d'))->translatedFormat('l'));
            });
        }, 'hari' => function($query) use ($absen){
            $query->where('nama', Carbon::create($absen->created_at->format('Y-m-d'))->translatedFormat('l'));
        ...
        if($jam_ptk){
            ...
            $masuk = Absen_masuk::updateOrCreate(
                ...
                [
                    ...
                    'created_at' => $absen->created_at->format('Y-m-d').' '.$jam_masuk
                ]
            );
            ...
            $pulang = Absen_pulang::updateOrCreate(
                ...
                [
                    ...
                    'created_at' => $absen->created_at->format('Y-m-d').' '.$jam_pulang
                ]
            );
        }
        $data = [
